...
unsigned int* dev_image_data, image_data[SCREEN_WIDTH * SCREEN_HEIGHT], * dev_image_sequence[FRAMECOUNT];
float* dev_zbuffer;
...
LRESULT CALLBACK WndProc0(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		for (i = 0; i < FRAMECOUNT; ++i) cudaMalloc((void**)&dev_image_sequence[i], SCREEN_WIDTH * SCREEN_HEIGHT * sizeof(unsigned int));
	...
	case WM_CLOSE:
		...
		for (i = 0; i < FRAMECOUNT; ++i) cudaFree(dev_image_sequence[i]);
...
void render_image_sequence(void)
{
	...
	for (i = 0; i < FRAMECOUNT; ++i)
	{
		...
		rot_degree_y = i * YROTANGLE * Math_PI / 180;// rot_degree_y2
		...
		cudaMemcpy(dev_image_sequence[i], dev_image_data, SCREEN_WIDTH * SCREEN_HEIGHT * sizeof(unsigned int), cudaMemcpyDeviceToDevice);
	}
